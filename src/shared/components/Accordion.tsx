...
const Accordion = () => {
	const [isShowFAQs, setIsShowFAQs] = useState(Array(5).fill(true));

	const accordionFAQsList1 = [
		{
	...
	return (
		<div>
			{accordionFAQsList1.map((list, i) => (
				<div key={i} className={i >= accordionFAQsList1.length - 1 ? "overflow-hidden" : "border-b overflow-hidden"}>
					<div className="flex justify-between items-center py-3.5" onClick={() => showFAQs(i)}>
						<h6 className="font-bold">{list.qus}</h6>
						<span className="text-2xl font-medium pb-1">{isShowFAQs[i] ? "+" : "-"}</span>
					</div>
					<div className={isShowFAQs[i] ? " h-0" : "h-full mb-4"}>
