	...
	return props.properties.map((property: any) => (
		<div key={property.id} className="border rounded-xl overflow-hidden shadow-sm cursor-pointer">
			<div className="h-48 min-h-192 rounded-t-at-regular overflow-hidden relative">
				<Image layout="fill" objectFit="cover" src={`https://resi.uatz.view.com.au/viewstatic/images/listing/${property.imageUrlSlug}/400-w/${property.heroImageUrl.split("/")[2]}`} alt={`${property.unitNumber ? property.unitNumber + "/" : ""}${property.streetNumber} ${property.streetName}, ${property.suburbName}`} />
			</div>
			<div className="p-4">
				<p className="text-base font-bold mr-2 mb-1.5">{property.priceText ? `${property.priceText}` : "Contact Agent"}</p>
